...
torch_compile_cache_dir = 'tmp/torch_compile_base'  # path where to save compiled kernels
...
inference_diffusion_steps = 2  # how many denoising steps to use for FAD calculation

fad_models = ['vggish', 'clap']  # list of FAD models to use
fad_workers = 16  # number of workers for FAD evaluation
fad_background_embeddings = [
    f'fad_stats/{data_path_test.replace("/", "")}_{fm}.npy'
    for fm in fad_models
]  # name of fad embeddings file. If does not exist, it will be created on the first run
...
multipliers_list = [1, 2, 4, 4,
                    4]  # base channels multipliers for each resolution level
attention_list = [
    0, 0, 1, 1, 1
]  # for each resolution, 0 if no attention is performed, 1 if attention is performed
freq_downsample_list = [
    2, 0, 0, 0
]  # for each resolution, 0 if frequency 4x downsampling, 1 if standard frequency 2x and time 2x downsampling

layers_list_encoder = [1, 1, 1, 1,
                       1]  # number of blocks per each resolution level
